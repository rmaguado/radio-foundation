...
class KoLeoLoss(nn.Module):
    ...
    def pairwise_NNs_inner(self, x) -> torch.Tensor:
        """
    ...
    def forward(self, student_output, eps=1e-8) -> torch.Tensor:
        """
        Args:
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            if dist.is_enabled():
                all_student_outputs = torch.cat(
                    torch.distributed.nn.all_gather(student_output), dim=0  # type: ignore
                )  # global_B x D
