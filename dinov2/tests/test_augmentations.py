...
def test_recursive_hierarchy(dummy_image3d, config):
    ...
    assert output["3d_global_ld"]["images"].shape == (1, 112, 112, 112)
    assert output["3d_local_ld"]["images"].shape == (1, 3, 56, 56, 56)
    assert output["2d_global_hd"]["images"].shape == (1, 3, 1, 224, 224)
    assert output["2d_local_hd"]["images"].shape == (1, 3, 2, 1, 112, 112)
