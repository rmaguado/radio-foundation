...
def sample_from_queues(
    ...
    cache_tokens = torch.stack([patch_embedding_cache[idx].to(device) for idx in cache_indices], dim=0)
    ...
    neg_tokens = torch.stack([negative_patch_queue[idx].to(device) for idx in neg_indices], dim=0)
...
def update_difficult_negatives(classifier_output, patch_labels, patch_tokens, negative_patch_queue):
