...
from evaluation.utils.networks import AttentionMapPatchPredictor
...
from evaluation.utils.train import train, evaluate, get_attention_maps
...
def get_dataloaders(args, label):
    ...
    embeddings_provider = CachedEmbeddings(embeddings_path, patch_only=args.patch_only)
...
def get_model(args, device):
    classifier_model = AttentionMapPatchPredictor(
        args.embed_dim,
        args.hidden_dim,
        num_labels=1,
...
def run_evaluation(args, label):
    ...
    for i in range(epochs):
        ...
        eval_metrics, _, attention_maps = get_attention_maps(
            classifier_model,
            val_loader,
            device=device,
        )
