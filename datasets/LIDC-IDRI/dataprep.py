...
class SeriesProcessor(SeriesProcessorBase):
    ...
    def preprocess_image(self, image, mask=None):
        resampled_image, resampled_mask, resampled_spacing = self.resample_image(
            image, mask
        )
    ...
    def process_series(self, scan, path_to_series, series_id, patient_id):
        ...
        segmentation_mask = np.zeros_like(sitk.GetArrayViewFromImage(image), dtype=bool)
        ...
        metadata = {
            ...
            "shape": None,
            "spacing": {"original": original_spacing, "resampled": None},
            "other": {"nodules_count": len(nods)},
        }
        ...
        image_array, resampled_mask, resampled_spacing = self.preprocess_image(
...
class LidcIdri(DatasetBase):
    ...
    def get_patient_ids(self):
        ...
        for patient_id in patient_ids:
            ...
            patient_ids_series += [
                (patient_id, scan.get_path_to_dicom_files(), scan) for scan in scans
            ]
    ...
    def prepare_dataset(self):
        ...
        for patient_id, series_path, scan in tqdm(patient_ids):
            ...
            image_array, mask_array, metadata = processor.process_series(
                scan, series_path, series_id, patient_id
            )
