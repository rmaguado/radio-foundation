...
class DataAugmentationDINO(object):
    def __init__(self, cfg, use_full_image: bool):
        ...
        self.global1, self.global2, self.local1 = load_transforms_from_cfg(
            cfg, use_full_image
        )
...
def load_transforms_from_cfg(cfg, use_full_image: bool):
    ...
    def build_transform_group(transform_group):
        transforms_list = [
            create_transform(tc) for tc in cfg.augmentations[transform_group]
        ]
    ...
    transform_groups = [
        build_transform_group(group) for group in ["global_1", "global_2", "local"]
    ]
