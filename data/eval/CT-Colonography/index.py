"""
Indexing script for evaluation with CT-Colonography
"""
    ...
    def insert_dataset_data(self, metadata: Tuple, dataset_name: str) -> None:
        ...
        self.cursor.execute(
            ...
            INSERT INTO {dataset_name_str} (
                series_id, patient_id, num_slices, image_shape_x, image_shape_y,
...
class Processor(DicomProcessor):
    ...
    def process_series(
        ...
        Args:
            ...
            patient_id (str): ID to link the scan to its metadata.
        """
    ...
    def get_paths_and_patient_id(self) -> List[Tuple[str, str]]:
        """
        Get paths to dicom files and their corresponding patient_ids.
        """
        paths_ids = []
        ...
        for patient_id in patient_ids:
            patient_id_path = os.path.join(self.absolute_dataset_path, patient_id)

            for root, _, files in os.walk(patient_id_path):
                if any(file.lower().endswith(".dcm") for file in files):
                    paths_ids.append((root, patient_id))
    ...
    def prepare_dataset(self) -> None:
        ...
        paths_ids = self.get_paths_and_patient_id()

        for scan_path, patient_id in tqdm(paths_ids):
            dcm_paths = [
            ...
            for series_id, dicoms in grouped_series.items():
                ...
                try:
                    self.process_series(dicoms, series_id, patient_id)
...
def main(args):
    dataset_name = "CT COLONOGRAPHY"
    db_name = "CT-Colonography"
